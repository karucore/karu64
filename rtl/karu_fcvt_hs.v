...
module karu_fcvt_hs (
    ...
    assign a_man = a[9:0];
    wire        a_zero;
    assign a_zero = (a_exp == 5'h00) && (a_man == 10'h0);
    wire        a_sub;
    assign a_sub = (a_exp == 5'h00) && (a_man != 10'h0);
    wire        a_inf;
    assign a_inf = (a_exp == 5'h1F) && (a_man == 10'h0);
    wire        a_nan;
    assign a_nan = (a_exp == 5'h1F) && (a_man != 10'h0);
    wire        a_snan;
    assign a_snan = a_nan && !a_man[9];
    wire        a_iz;
    assign a_iz = a_zero;
    ...
    wire [3:0]  a_clz;
    assign a_clz = a_sub ? clz10(a_man) : 4'd0;
    wire [9:0]  a_man_n;
    assign a_man_n = a_sub ? (a_man << (a_clz + 4'd1)) : a_man;

    //  Re-bias H->S: exp_s = a_exp - 15 + 127 = a_exp + 112 (normal).
    //  Normalized subnormal has biased S exp 112 - a_clz.
    wire [7:0]  exp_s;
    assign exp_s = a_sub ? (8'd112 - {4'b0, a_clz})
                            : ({3'b0, a_exp} + 8'd112);
    wire [22:0] mant_s;
    assign mant_s = {a_man_n, 13'b0};
...
module karu_fcvt_sh (
    ...
    wire        a_iz;        //  true zero short-circuits to signed zero
    assign a_iz = a_zero;

    //  Re-bias S->H: exp_h = a_exp - 127 + 15 = a_exp - 112.
    wire signed [10:0] exp_unb;
    assign exp_unb = $signed({3'b0, a_exp}) - 11'sd127;
    wire signed [10:0] exp_h_pre;
    assign exp_h_pre = exp_unb + 11'sd15;

    //  H mantissa = top 10 of S mantissa; bits [12:0] are below it.
    wire [9:0]  man_h_unround;
    assign man_h_unround = a_man[22:13];
    wire        round_bit;
    assign round_bit = a_man[12];
    wire        sticky_bit;
    assign sticky_bit = |a_man[11:0];
    ...
    wire [11:0] man_rnd;
    assign man_rnd = {1'b0, 1'b1, man_h_unround} + {11'b0, round_up};
    wire        man_carry;
    assign man_carry = man_rnd[11];
    wire        inexact;
    assign inexact = round_bit || sticky_bit;
    wire [9:0]  man_trunc;
    assign man_trunc = man_carry ? 10'b0 : man_rnd[9:0];
    wire [9:0]  man_final;
    assign man_final = is_rod ? (man_h_unround | {9'b0, inexact}) : man_trunc;
    wire signed [10:0] exp_final;
    assign exp_final = (man_carry && !is_rod) ? (exp_h_pre + 11'sd1) : exp_h_pre;

    wire over;    //  H exp 0x1F == inf
    assign over = (exp_final >= 11'sd31);
    wire under;     //  below H min normal
    assign under = (exp_final <= 11'sd0);
    ...
    wire signed [10:0] sub_shift;
    assign sub_shift = 11'sd1 - exp_h_pre;
    wire is_total_under;   //  below 2^-25 (mant_bits+2)
    assign is_total_under = (sub_shift >= 11'sd12);
    wire is_subn;
    assign is_subn = (exp_h_pre <= 11'sd0) && !is_total_under;

    wire [23:0] mfull_24;
    assign mfull_24 = {1'b1, a_man[22:0]};
    wire [10:0] R;            //  13 = 23 - 10 (mant offset)
    assign R = 11'sd13 + sub_shift;
    wire [63:0] mfull_xx;
    assign mfull_xx = {40'b0, mfull_24};
    wire [63:0] msh;
    assign msh = (R >= 11'sd64) ? 64'b0 : (mfull_xx >> R[5:0]);
    wire [9:0]  sub_mant_raw;
    assign sub_mant_raw = msh[9:0];

    wire sub_round;
    assign sub_round = (R >= 11'sd1 && R <= 11'sd24) ? mfull_24[R[4:0] - 5'd1] : 1'b0;
    ...
    assign sub_sticky_mask =
        ...
        24'b0;
    wire sub_sticky;
    assign sub_sticky = |(mfull_24 & sub_sticky_mask);
    ...
    wire [10:0] sub_mant_rounded;
    assign sub_mant_rounded = {1'b0, sub_mant_raw} + {10'b0, sub_round_up};
    wire        sub_mant_carry; //  -> smallest normal
    assign sub_mant_carry = !is_rod && sub_mant_rounded[10];
    wire [9:0]  sub_mant_final;
    assign sub_mant_final = is_rod ? (sub_mant_raw | {9'b0, sub_inexact})
    ...
    wire [15:0] under_total_res;
    assign under_total_res = under_total_away ? {a_sign, 5'h00, 10'h1}
                                              : {a_sign, 15'b0};
...
module karu_fcvt_dh (
    ...
    wire signed [12:0] exp_h_pre;
    assign exp_h_pre = exp_unb + 13'sd15;

    //  H mantissa = top 10 of the 52-bit D mantissa; [41:0] are below it.
    wire [9:0]  man_h_unround;
    assign man_h_unround = a_man[51:42];
    wire        round_bit;
    assign round_bit = a_man[41];
    wire        sticky_bit;
    assign sticky_bit = |a_man[40:0];
    ...
    assign exp_final = (man_carry && !is_rod) ? (exp_h_pre + 13'sd1) : exp_h_pre;

    wire over;
    assign over = (exp_final >= 13'sd31);
    wire under;
    assign under = (exp_final <= 13'sd0);

    //  ---------- subnormal output ----------
    wire signed [12:0] sub_shift;
    assign sub_shift = 13'sd1 - exp_h_pre;
    wire is_total_under;   //  mant_bits + 2
    assign is_total_under = (sub_shift >= 13'sd12);
    wire is_subn;
    assign is_subn = (exp_h_pre <= 13'sd0) && !is_total_under;

    wire [52:0] mfull_53;
    assign mfull_53 = {1'b1, a_man[51:0]};
    wire [12:0] R;            //  42 = 52 - 10 (mant offset)
    assign R = 13'sd42 + sub_shift;
    wire [127:0] mfull_xx;
    ...
    wire sub_round;
    assign sub_round = (R >= 13'sd1 && R <= 13'sd53) ? mfull_53[R[5:0] - 6'd1] : 1'b0;
