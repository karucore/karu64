...
module karu_f2i (
    ...
    //  Unbiased exponent
    wire signed [9:0] exp_unb;
    assign exp_unb = {2'b0, a_exp} - 10'sd127;
    ...
    wire signed [9:0] shift_left;  //  positive: shift left
    assign shift_left = exp_unb - 10'sd23;
    wire signed [9:0] shift_right;  //  positive: shift right
    assign shift_right = 10'sd23 - exp_unb;
    ...
    wire [95:0] base96;
    assign base96 = {72'b0, mant_full};
    wire        sl_overflow; //  mantissa exits base96
    assign sl_overflow = (shift_left  >= 10'sd73);
    wire        sr_too_far; //  result mag is 0
    assign sr_too_far = (shift_right >= 10'sd96);
    wire [95:0] shifted_l;
    assign shifted_l = base96 << shift_left[6:0];
    wire [95:0] shifted_r;
    assign shifted_r = sr_too_far ? 96'b0 : (base96 >> shift_right[6:0]);

    wire is_left;
    assign is_left = (exp_unb >= 10'sd23);
    wire [95:0] shifted;
    ...
    wire round_bit;
    assign round_bit = is_left ? 1'b0 : ((shift_right >  10'sd0 && shift_right <= 10'sd24)
    ...
    wire        overflow_bits;
    assign overflow_bits = sl_overflow || |shifted[95:64];
    ...
    wire        special_high;
    assign special_high = a_nan || a_inf || overflow_bits || mag_carry;
    ...
    wire signed_neg_max_ok;
    assign signed_neg_max_ok = a_sign && (mag_rnd[63:0] == 64'h8000_0000_0000_0000) && is_long;
    wire signed_neg_max_ok_w;
    assign signed_neg_max_ok_w = a_sign && (mag_rnd[63:0] == 64'h0000_0000_8000_0000) && !is_long;
    ...
    wire [63:0] base_result;   //  W result always sign-extended
    assign base_result =
        a_is_zero ? 64'b0 :
        a_nan ? nan_value :
        any_out ? sat_value :
...
module karu_i2f (
    ...
    wire [6:0] lz;
    assign lz = clz64(mag);
    wire [6:0] msb_pos;        //  position of leading 1
    ...
    wire [24:0] mant_rnd;
    assign mant_rnd = {1'b0, mant_full} + {24'b0, round_up};
    wire        mant_carry;
    assign mant_carry = mant_rnd[24];
    wire [22:0] mant_final;
    assign mant_final = mant_carry ? 23'b0 : mant_rnd[22:0];
    wire [7:0]  exp_final;
