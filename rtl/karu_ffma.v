...
module karu_ffma (
    ...
    // ================================================================
    wire        signA; assign signA = a[31];  wire [7:0] expA_r; assign expA_r = a[30:23];  wire [22:0] sigA_r;
    assign sigA_r = a[22:0];
    wire        signB; assign signB = b[31];  wire [7:0] expB_r; assign expB_r = b[30:23];  wire [22:0] sigB_r;
    assign sigB_r = b[22:0];
    wire        signC;
    assign signC = c[31] ^ neg_c;
    wire [7:0]  expC_r; assign expC_r = c[30:23];  wire [22:0] sigC_r;
    assign sigC_r = c[22:0];
    wire        signProd;
    assign signProd = signA ^ signB ^ neg_prod;

    wire a_is_nan;
    assign a_is_nan = (expA_r == 8'hFF) && (sigA_r != 0);
    wire b_is_nan;
    assign b_is_nan = (expB_r == 8'hFF) && (sigB_r != 0);
    wire c_is_nan;
    assign c_is_nan = (expC_r == 8'hFF) && (sigC_r != 0);
    wire a_snan;
    assign a_snan = a_is_nan && !sigA_r[22];
    wire b_snan;
    assign b_snan = b_is_nan && !sigB_r[22];
    wire c_snan;
    assign c_snan = c_is_nan && !sigC_r[22];
    wire a_inf;
    assign a_inf = (expA_r == 8'hFF) && (sigA_r == 0);
    wire b_inf;
    assign b_inf = (expB_r == 8'hFF) && (sigB_r == 0);
    wire c_inf;
    assign c_inf = (expC_r == 8'hFF) && (sigC_r == 0);
    ...
    wire [6:0]  a_sd;
    assign a_sd = clz32({9'b0, sigA_r}) - 7'd8;
    wire [6:0]  b_sd;
    assign b_sd = clz32({9'b0, sigB_r}) - 7'd8;
    wire [6:0]  c_sd;
    assign c_sd = clz32({9'b0, sigC_r}) - 7'd8;
    wire signed [15:0] expA;
    assign expA = a_sub ? (16'sd1 - {{9{1'b0}}, a_sd}) : $signed({8'b0, expA_r});
    wire signed [15:0] expB;
    assign expB = b_sub ? (16'sd1 - {{9{1'b0}}, b_sd}) : $signed({8'b0, expB_r});
    wire signed [15:0] expC;
    assign expC = c_sub ? (16'sd1 - {{9{1'b0}}, c_sd}) : $signed({8'b0, expC_r});
    wire [22:0] sigA_n;
    ...
    wire inf_minus_inf;
    assign inf_minus_inf = prod_is_inf && c_inf && (signProd != signC);
    wire nv;
    ...
    wire cancel_zero;
    assign cancel_zero = c_is_zero && (signProd != signC);
    wire [31:0] zeroprod_res;
    assign zeroprod_res =
        cancel_zero ? {(rm == `FRM_RDN), 31'b0} : {signC, c[30:0]};

    wire signed [15:0] expProd0;
    assign expProd0 = expA + expB - 16'sd126;
    wire [30:0] sigA31;
    assign sigA31 = {1'b1, sigA_n, 7'b0};
    wire [30:0] sigB31;
    assign sigB31 = {1'b1, sigB_n, 7'b0};
    ...
    reg [2:0]           s0_rm;
    wire [63:0] s1_sigProd0_w;
    assign s1_sigProd0_w = {33'b0, s0_sigA31} * {33'b0, s0_sigB31};
    ...
    // ================================================================
    wire        prod_lt;
    assign prod_lt = (s1_sigProd0 < 64'h2000_0000_0000_0000);
    wire signed [15:0] expProd;
    assign expProd = prod_lt ? (s1_expProd0 - 16'sd1) : s1_expProd0;
    wire [63:0] sigProd;
    assign sigProd = prod_lt ? (s1_sigProd0 << 1) : s1_sigProd0;

    wire [31:0] sigC30;
    assign sigC30 = {2'b0, 1'b1, s1_sigC_n, 6'b0};
    wire [63:0] sig64C;
    assign sig64C = {sigC30, 32'b0};
    ...
    wire same_sign;
    assign same_sign = (s1_signProd == s1_signC);

    wire [31:0] d_add_le;
    assign d_add_le = 32'd32 - expDiff32;
    wire [31:0] d_add_gt;
    assign d_add_gt = expDiff32;
    wire [31:0] d_sub_lt;
    assign d_sub_lt = -expDiff32;
    wire [31:0] d_sub_gt;
    assign d_sub_gt = expDiff32;

    wire [63:0] srj_add_le;
    assign srj_add_le = srj64(sigProd, d_add_le);
    wire [31:0] add_sigZ_le;
    assign add_sigZ_le = sigC30 + srj_add_le[31:0];
    wire [63:0] srj_add_gt;
    assign srj_add_gt = srj64(sig64C, d_add_gt);
    wire [63:0] add_sig64_gt;
    assign add_sig64_gt = sigProd + srj_add_gt;
    wire [63:0] srj_add_gt2;
    assign srj_add_gt2 = srj64(add_sig64_gt, 32'd32);
    wire [31:0] add_sigZ_gt;
    assign add_sigZ_gt = srj_add_gt2[31:0];
    wire        add_le;
    assign add_le = (expDiff <= 0);
    wire signed [15:0] add_expZ0;
    assign add_expZ0 = add_le ? s1_expC : expProd;
    wire [31:0] add_sigZ0;
    assign add_sigZ0 = add_le ? add_sigZ_le : add_sigZ_gt;
    wire        add_norm;
    assign add_norm = (add_sigZ0 < 32'h4000_0000);
    wire signed [15:0] add_expZ;
    assign add_expZ = add_norm ? (add_expZ0 - 16'sd1) : add_expZ0;
    wire [31:0] add_sigZ;
    assign add_sigZ = add_norm ? (add_sigZ0 << 1) : add_sigZ0;

    wire [63:0] srj_sub_lt;
    assign srj_sub_lt = srj64(sigProd, d_sub_lt);
    wire [63:0] sub_sig64_lt;
    assign sub_sig64_lt = sig64C - srj_sub_lt;
    wire [63:0] sub_sig64_eq;
    assign sub_sig64_eq = sigProd - sig64C;
    wire [63:0] srj_sub_gt;
    assign srj_sub_gt = srj64(sig64C, d_sub_gt);
    wire [63:0] sub_sig64_gt;
    assign sub_sig64_gt = sigProd - srj_sub_gt;
    wire        sub_eq_zero;
    assign sub_eq_zero = (expDiff == 0) && (sub_sig64_eq == 64'b0);
    wire        eq_neg;
    assign eq_neg = (expDiff == 0) && sub_sig64_eq[63];
    wire [63:0] sub_sig64;
    assign sub_sig64 =
    ...
    assign sub_signZ =
        ...
        (expDiff == 0) ? (eq_neg ? !s1_signProd : s1_signProd) :
                         s1_signProd;
    wire signed [15:0] sub_expZ_base;
    assign sub_expZ_base = (expDiff < 0) ? s1_expC : expProd;

    wire        use_czero;
    assign use_czero = s1_c_is_zero;
    wire [63:0] srj_cz;
    assign srj_cz = srj64(sigProd, 32'd31);
    ...
    // ================================================================
    wire [6:0]  sub_clz;
    assign sub_clz = clz64(s2_sub_sig64);
    wire signed [15:0] sub_shiftDist;
    assign sub_shiftDist = {{9{1'b0}}, sub_clz} - 16'sd1;
    wire signed [15:0] sub_expZ;
    assign sub_expZ = s2_sub_expZ_base - sub_shiftDist;
    wire signed [15:0] sub_sh2;
    assign sub_sh2 = sub_shiftDist - 16'sd32;
    wire [31:0] neg_sub_sh2;
    assign neg_sub_sh2 = -{{16{sub_sh2[15]}}, sub_sh2};
    wire [63:0] srj_sub2;
    assign srj_sub2 = srj64(s2_sub_sig64, neg_sub_sh2);
    wire [63:0] sub_sig64_l;
    assign sub_sig64_l = s2_sub_sig64 << sub_sh2[5:0];
    wire [31:0] sub_sigZ;
    assign sub_sigZ = (sub_sh2 < 0) ? srj_sub2[31:0] : sub_sig64_l[31:0];

    wire        rp_sign_w;
    assign rp_sign_w = s2_use_czero ? s2_signProd : (s2_same_sign ? s2_signProd : s2_sub_signZ);
    wire signed [15:0] rp_exp_w;
    assign rp_exp_w = s2_use_czero ? s2_cz_exp
                  : (s2_same_sign ? s2_add_expZ : sub_expZ);
    wire [31:0] rp_sig_w;
    assign rp_sig_w = s2_use_czero ? s2_cz_sig
                  : (s2_same_sign ? s2_add_sigZ : sub_sigZ);
    wire        complete_cancel_w;
    assign complete_cancel_w = !s2_special_active && !s2_zero_prod && !s2_use_czero
    ...
    // ================================================================
    wire roundNearEven;
    assign roundNearEven = (s3_rm == `FRM_RNE);
    wire roundNearMax;
    assign roundNearMax = (s3_rm == `FRM_RMM);
    ...
    wire enter_block;
    assign enter_block = (s3_rp_exp < 0) || (s3_rp_exp >= 16'sd253);
    wire subn;
    assign subn = enter_block && (s3_rp_exp < 0);
    wire [31:0] srj_amt;
    assign srj_amt = -{{16{s3_rp_exp[15]}}, s3_rp_exp};
    wire [31:0] sub_sig_sh;
    assign sub_sig_sh = srj32(s3_rp_sig, srj_amt);
    wire [31:0] eff_sig;
    assign eff_sig = subn ? sub_sig_sh : s3_rp_sig;
    wire signed [15:0] eff_exp;
    assign eff_exp = subn ? 16'sd0 : s3_rp_exp;
    wire [6:0] roundBits;
    assign roundBits = eff_sig[6:0];

    wire [32:0] sig_plus_inc;
    assign sig_plus_inc = {1'b0, s3_rp_sig} + {25'b0, roundIncrement};
    wire isTiny;
    assign isTiny = subn && ((s3_rp_exp < -16'sd1) || (sig_plus_inc < 33'h0_8000_0000));
    wire overflow;
    assign overflow = enter_block && !subn
                   && ((s3_rp_exp > 16'sd253) || (sig_plus_inc >= 33'h0_8000_0000));

    wire [32:0] sig_rnd;
    assign sig_rnd = ({1'b0, eff_sig} + {25'b0, roundIncrement}) >> 7;
    wire        tie_even;
    assign tie_even = (roundBits == 7'h40) && roundNearEven;
    wire [31:0] sig_final0;
    assign sig_final0 = sig_rnd[31:0] & ~(tie_even ? 32'b1 : 32'b0);
    wire [31:0] exp_for_pack;
    assign exp_for_pack = (sig_final0 == 0) ? 32'b0 : {16'b0, eff_exp};
    wire [31:0] rp_packed;
    assign rp_packed = {s3_rp_sign, 31'b0} + (exp_for_pack << 23) + sig_final0;
    ...
    wire [31:0] over_res;
    assign over_res = {s3_rp_sign, 8'hFF, 23'h0} - ((roundIncrement == 0) ? 32'b1 : 32'b0);

    wire [31:0] roundpack_res;
    ...
    // ================================================================
    wire [23:0] sigA24;      //  {1'b1, sigA_n} (combinational at req)
    assign sigA24 = sigA31[30:7];
    wire [23:0] sigB24;
    assign sigB24 = sigB31[30:7];
    wire        mul_done;
    wire [63:0] mul_prod;
    ...
    if (F_FMA_CYCLES == 1) begin : g_fma_comb
        ...
        reg [47:0] sacc;
        reg [23:0] sma;
        wire [F_FMA_K+23:0] smul_partial;
        assign smul_partial = sma * sacc[F_FMA_K-1:0];
        wire [F_FMA_K+23:0] smul_sum;
        assign smul_sum = sacc[47:24] + smul_partial;
        wire [47:0]         smul_next;
        assign smul_next = { smul_sum, sacc[23:F_FMA_K] };
